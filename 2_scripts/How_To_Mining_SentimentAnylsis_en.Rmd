...
With this argument you can define the language of the Tweets. For instance, *lang = "en"* indicates that only english Tweets should be looked for. 

### Example

The following example indicates to look for Tweets, that contain the keywords *Roger* or *Federer* (or change them if you want). 10 Tweets should be acquired for this demonstration and they should be in english. The geocode indicates Switzerland. We do not want to include retweets and recent but popular Tweets should be searched. 
...
In this case, we have use an English lexicon. 
There are multiple lexicon to choose from. 
The r package *lexicon* therefore comes in handy.
In this example we use the large lexicon sentiword. 

Load the package *lexicon* (maybe install it before if you have not done that) and then the lexicon is automatically in your environment. 
...
Now we acquired the Lexicon sentiwords. For further usage lets put it in the local environment and rename it to lexikey. Take a look if you want to. The x column indicates the words, the y column the weight or intensity.
